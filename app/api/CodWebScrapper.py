...
def getLatestMatch(gamertag,
    ...
    try:
        ...
        return JSON  #This will return the JSON return of a latest match request. By default it lists the latest 20 matches. The below for loop would retrieve the first one in the list. .get is how to retrieve one value from JSON.
    except:
...
def getLatestMatchStats(gamertag, platform,
    ...
    return JSON  #Returns JSON of requested information. JSON Requests for match stats returns all stats for all players. The Print example below is how to get one such players stats.

    #print('Player ID: {}\nkills: {}\ndeaths: {}\nkdRatio: {}\nscore: {}\nscorePerMinute: {}\n'.format(gamertag,stat.get('kills'),stat.get('deaths'),stat.get('kdRatio'),stat.get('score'),stat.get('scorePerMinute')))
    #print(JSON["data"]["summary"]) Use This for JSON of All Possible stats. Also is the original return for the JSON request.
